		...
		if [ "${update_id[$id]}" ]; then
			...
			[[ $download_file -eq 1 ]] && {
				...
				file_path=$(ShellBot.downloadFile --file_path "${file_final}" --dir "$dest_file") && {
					...
					message=$(sudo docker run --rm -i -v ${PWD}:/home/tensor-photos tensorflow python /home/tensor-example.py "/home/tensor-photos/$(echo $file_path | cut -d'|' -f2 | sed 's#\.\/##')" > /tmp/usar_urandom.log)
